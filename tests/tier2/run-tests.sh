...
RESULTS="${RESULTS:-$SCRIPT_DIR/results.json}"
...
run_query() {
    ...
    # Run memory search
    local search_result
    search_result=$(openclaw memory search "$query" --agent "$AGENT" --json 2>/dev/null || echo '{"results":[]}')
    
    # Parse results
    local got_results
    got_results=$(echo "$search_result" | jq -c '[.results[:5] | .[] | {text: .snippet, score: .score, file: .path}]' 2>/dev/null || echo '[]')
...
main() {
    echo ""
    echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║     Tier 2 Memory Retrieval Test Harness                  ║${NC}"
    echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
    echo ""
